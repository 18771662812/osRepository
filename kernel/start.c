...
// static inline uint64 r_menvcfg() { uint64 x; __asm__ volatile("csrr %0, menvcfg" : "=r" (x)); return x; }
// static inline void w_menvcfg(uint64 x) { __asm__ volatile("csrw menvcfg, %0" :: "r" (x)); }
...
static void timerinit_xv6_like() {
    ...
    // w_menvcfg(r_menvcfg() | (1UL << 63)); // sstc
...
void start() {
    ...
    // delegate traps/interrupts to S mode
    w_medeleg(0xffff);
    w_mideleg(0xffff);
